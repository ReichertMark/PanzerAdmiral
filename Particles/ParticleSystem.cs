...
namespace PanzerAdmiral.Particles
{
    ...
    public abstract class ParticleSystem
    {
        ...
        /// "constants to be set by subclasses", which give each ParticleSystem its specific flavor.
        /// </summary>
        ...
        public void LoadContent()
        {
            ...
            if (string.IsNullOrEmpty(textureFilename))
            {
                string message = "textureFilename wasn't set properly, so the " +
                    ...
                    "sure your particle system's InitializeConstants function properly sets textureFilename.";
                throw new InvalidOperationException(message);
            }
            // load the texture....
            texture = ScreenManager.Content.Load<Texture2D>(textureFilename);
        ...
        public void AddParticles(Vector2 where)
        {
            ...
            int numParticles = Extensions.Random.Next(minNumParticles, maxNumParticles);
        ...
        protected virtual void InitializeParticle(Particle p, Vector2 where)
        {
            ...
            float velocity = Extensions.RandomBetween(minInitialSpeed, maxInitialSpeed);
            float acceleration = Extensions.RandomBetween(minAcceleration, maxAcceleration);
            float lifetime = Extensions.RandomBetween(minLifetime, maxLifetime);
            float scale = Extensions.RandomBetween(minScale, maxScale);
            float rotationSpeed = Extensions.RandomBetween(minRotationSpeed, maxRotationSpeed);
        ...
        /// <param name="p">the particle to initialize</param>
        /// <param name="where">the position on the screen that the particle should be</param>
        /// <param name="setDirection">the direction of the particle</param>
        protected virtual void InitializeParticle(Particle p, Vector2 where, Vector2 setDirection)
        {
            // first, call PickRandomDirection to figure out which way the particle
            // will be moving. velocity and acceleration's values will come from this.
            Vector2 direction = setDirection; //PickRandomDirection();
        ...
        /// <summary>
        /// PickRandomDirection is used by InitializeParticles to decide which direction particles will move. 
        /// The default implementation is a random vector in a circular pattern.
        /// </summary>
        protected virtual Vector2 PickRandomDirection()
        {
            float angle = Extensions.RandomBetween(0, MathHelper.TwoPi);
        ...
        public void Update(GameTime gameTime)
        {
            ...
            foreach (Particle p in particles)
            {
                if (p.Active)
                {
                    ...
                    // if that update finishes them, put them onto the free particles queue.
                    if (!p.Active)
        ...
        #region Draw
        /// <summary>
        ///Draw will use Screenmanager's sprite batch to render all of the active particles.
        /// </summary>
        public void Draw(GameTime gameTime)
        {
            // tell sprite batch to begin, using the spriteBlendMode specified in initializeConstants
            ScreenManager.SpriteBatch.Begin(SpriteSortMode.Deferred, blendState);
            ...
            foreach (Particle p in particles)
            {
                ...
                Color color = ParticleColor * alpha; //Color.White * alpha;

                // make particles grow as they age. they'll start at 75% of their size,
                // and increase to 100% once they're finished.
                float scale = p.Scale * (.75f + .25f * normalizedLifetime);

                ScreenManager.SpriteBatch.Draw(texture, p.Position, null, color,
